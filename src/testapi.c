...
#include "httpcgi.h"
#include "testapi.h"
#include "consapi.h"        /* console_lock_acquire/release -- fn=cmd shares it (#214) */
#include "mvsmfmsg.h"
...
int testHandler(Session *session) {
  ...
  if (strcmp(fn, "listds") == 0) {
    ...
    /* Same correlation lock the console API takes (#214): this is a second
     * SVC 34 caller in the same address space and under the same MTT source, so
     * an unbracketed echo here lands inside an open /restconsoles block. */
    if (console_lock_acquire(session) != 0) {
      rc = http_printf(session->httpc,
                       "cmd probe: console lock busy, not issuing\r\n");
      goto quit;
    }
