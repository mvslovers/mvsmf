 ...
 *
 * These tables provide correct round-trip translation between ASCII
 * (ISO 8859-1) and EBCDIC (IBM-1047). They replace the HTTPD server's
 * built-in tables which use a non-standard mapping that breaks characters
 * like the pipe '|' (ASCII 0x7C <-> EBCDIC 0x4F).
 */

/* ASCII (ISO 8859-1) -> EBCDIC (IBM-1047) */
const unsigned char asc2ebc_1047[256] = {
	/* 0x00-0x0F */
...
/* EBCDIC (IBM-1047) -> ASCII (ISO 8859-1) */
const unsigned char ebc2asc_1047[256] = {
	/* 0x00-0x0F */
...
void
mvsmf_atoe(unsigned char *buf, int len)
{
	register int i;

	for (i = 0; i < len; i++) {
		buf[i] = asc2ebc_1047[buf[i]];
	}
}

void
mvsmf_etoa(unsigned char *buf, int len)
{
	register int i;

	for (i = 0; i < len; i++) {
		buf[i] = ebc2asc_1047[buf[i]];
	}
}
