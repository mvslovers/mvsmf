...
# The read and stat blocks need a file that is already on the target, and the
# write/create/delete blocks need a path they may have. Both come from a fixture
# this suite uploads itself, into a directory of its own that it removes again
# (#283) -- the same shape zowe-uss.sh has always had.
#
# It used to be USS_TEST_FILE from .env, pointing at whatever happened to be on
# the system. /tmp is scratch space, so that path goes away sooner or later, and
# when it did the suite skipped five blocks and still reported every remaining
# assertion green. The stat-query block was skipped that way for as long as it
# took someone to read the NOTE line.
FIXTURE_SRC="${SCRIPT_DIR}/data/uss-fixture.txt"
WORK_DIR="/tmp/curl-uss-test-$$"
TEST_FILE="${WORK_DIR}/fixture.txt"
...
# =========================================================================
# 0. Fixture — upload tests/data/uss-fixture.txt into a directory of our own
#
# Everything from here on assumes both exist, so a failure is fatal rather than
# a skip: a suite that quietly drops five blocks is what #283 is about. The
# fixture goes up in text mode, which is how the read blocks below read it back,
# so the bytes on the wire are the ones in the repo file.
# =========================================================================

trap 'cleanup_recursive "${WORK_DIR}"' EXIT

if [ ! -f "$FIXTURE_SRC" ]; then
	echo "ERROR: fixture ${FIXTURE_SRC} is missing from the repo."
	exit 1
fi

cleanup_recursive "${WORK_DIR}"

HTTP_CODE=$(curl -s -o /dev/null -w '%{http_code}' -X POST -u "$AUTH" \
	-H "Content-Type: application/json" \
	-d '{"type":"directory","mode":"rwxr-xr-x"}' \
	"${BASE_URL}/zosmf/restfiles/fs${WORK_DIR}")
if [ "$HTTP_CODE" != "201" ]; then
	echo "ERROR: could not create ${WORK_DIR} (HTTP ${HTTP_CODE})."
	echo "The suite needs it for every file block -- is UFSD running?"
	exit 1
fi

HTTP_CODE=$(curl -s -o /dev/null -w '%{http_code}' -X PUT -u "$AUTH" \
	-H "Expect:" --data-binary "@${FIXTURE_SRC}" \
	"${BASE_URL}/zosmf/restfiles/fs${TEST_FILE}")
if [ "$HTTP_CODE" != "204" ]; then
	echo "ERROR: could not upload the fixture to ${TEST_FILE} (HTTP ${HTTP_CODE})."
	exit 1
fi
...
TEXT_OUT=$(mktemp)
HTTP_CODE=$(curl -s -o "$TEXT_OUT" -w '%{http_code}' \
	-u "$AUTH" \
	"${BASE_URL}/zosmf/restfiles/fs${TEST_FILE}")

assert_http_status "200" "$HTTP_CODE" "read file text mode ${TEST_FILE}"

# The fixture went up in text mode and comes back in text mode, so the bytes
# must be the repo file's -- byte for byte, not merely non-empty. Line 3 of the
# fixture is the reason: it carries [ ] ^ ! @ # $ | ~ \ { } `, the characters
# CP037 and IBM-1047 disagree about. A handler translating a USS payload with
# the dataset table corrupts exactly those and nothing else, returns 200, and
# is invisible to a length or non-empty check (see the Codepage Override in
# CLAUDE.md).
if cmp -s "$FIXTURE_SRC" "$TEXT_OUT"; then
	pass "read file: text-mode round-trip is byte-identical to the fixture"
else
	fail "read file: text-mode round-trip is byte-identical to the fixture" \
		"$(cmp "$FIXTURE_SRC" "$TEXT_OUT" 2>&1 | head -1)"
fi
rm -f "$TEXT_OUT"
...
BIN_OUT=$(mktemp)
HTTP_CODE=$(curl -s -o "$BIN_OUT" -w '%{http_code}' \
	-u "$AUTH" \
...
# Binary mode returns the stored bytes untranslated, so this must NOT equal the
# fixture -- the file is stored in EBCDIC. Asserting the length instead: same
# byte count, different bytes. Equality here would mean the payload was never
# translated on the way in.
FIX_LEN=$(wc -c < "$FIXTURE_SRC" | tr -d ' ')
BIN_LEN=$(wc -c < "$BIN_OUT" | tr -d ' ')
if [ "$BIN_LEN" = "$FIX_LEN" ] && ! cmp -s "$FIXTURE_SRC" "$BIN_OUT"; then
	pass "read file: binary mode returns ${BIN_LEN} untranslated bytes"
else
	fail "read file: binary mode returns the stored EBCDIC bytes" \
		"length ${BIN_LEN} vs ${FIX_LEN}, identical=$(cmp -s "$FIXTURE_SRC" "$BIN_OUT" && echo yes || echo no)"
fi
rm -f "$BIN_OUT"
...
# path" block below, next to the write side of the same NULL open, rather
# than here. Aiming it at TEST_DIR was what hid issue #269 -- see the
# comment there.
...
# The GET side of the same NULL open, on the same fixture -- kept next to
# the write side so the two diagnoses of one condition stay together.
HTTP_CODE=$(curl -s -w '%{http_code}' -o /dev/null \
	-u "$AUTH" \
	"${BASE_URL}/zosmf/restfiles/fs${DIR_FIXTURE}")
